...
var current_state = possible_states.StartingExperiment; // One array for each block
...
var timer_rest = null; // Current block
...
function trialStarts(n) {
  ...
  $("#trials-left").html(n - 1); // Restart the trial object

  trial = Object.assign({}, trial_aux);
  trial.seq_timestamps = new Array(); // For the timer

  var starting_date = new Date().getTime(); // Add the initial timestamp

  trial.initial_timestamp = starting_date;
  trial.time_limit = blocks[current_block].time_per_trial;
  $("#time-left").html(trial.time_limit); // console.log(trial.time_limit)
  // Trial ends after X milliseconds

  setTimeout(function () {
    return trialEnds(n);
  }, trial.time_limit * 1000);
  $(document).keydown(keydownEvent); // Show timer
...
function trialEnds(n) {
  ...
  $(document).off("keydown"); // Reset timer

  $("#time-left").html(trial.time_limit);
  clearInterval(timer);
  timer = null;
  block_trials.push(Object.assign({}, trial)); // console.log(JSON.stringify(trials))

  if (n - 1 > 0) trialStarts(n - 1);else {
...
function sendData() {
  ...
  $.ajax({
    ...
    success: function success(data) {
      console.log(data); // Clear old trials, to avoid sending them more than one time.
...
$(document).ready(function () {
  ...
  $("#start-btn").click(function () {
    return setInterval(doExperiment, 100);
  });
});
...
function showStartMessage() {
  ...
  var subs = function subs(next) {
    $("#start-message-number").text(seconds_left.toString());
    seconds_left--;
    next();
  };

  $("#div-content").append("<p id='start-message'>Starting experiment in ... <b id='start-message-number'></b></p>").queue(subs).delay(1000).queue(subs).delay(1000).queue(subs).delay(1000).queue(function (next) {
    $("#start-message").remove();
    next();
  }).queue(function () {
    return current_state = possible_states.StartBlock;
  });
}

function doRest() {
  $("#time-left-rest").html(resting_time); // For the timer

  var starting_date = new Date().getTime(); // Show html

  $("#rest-info").show(); // Rest ends after X milliseconds

  setTimeout(function () {
    current_state = possible_states.EndBlock;
    $("#rest-info").hide();
  }, resting_time * 1000); // Show timer

  timer_rest = setInterval(function () {
    var now = new Date().getTime();
...
function doExperiment(next) {
  switch (current_state) {
    ...
    case possible_states.StartBlock:
      ...
      $("#sequence").html(sequences[current_block]); // fill the trial object accordingly

      current_state = possible_states.DoingTrial;
      runTrials(); //   console.log('Holaaa');
      // Start the trials
    ...
    case possible_states.EndBlock:
      ...
      timer_rest = null;
      console.log(block_trials); // Add block trials to array

      experiment_trials.push(block_trials); // Clean block trials

      block_trials = new Array();
      current_block++;
      if (current_block + 1 > blocks.length) current_state = possible_states.EndExperiment;else current_state = possible_states.StartBlock;
      break;
